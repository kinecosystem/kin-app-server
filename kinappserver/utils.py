...
def is_valid_client(user_id, validation_token):
    ...
    log.info('os_type: %s\napk_version: %s\APK_VERSION: %s' % (os_type, apk_version, validation_module.APK_VERSION))

    if os_type == OS_ANDROID and apk_version >= validation_module.APK_VERSION:
        log.info('REQUIRES VALIDATION')
        if validation_token is None:
            log.info("validation_token is None!")
            return False

        if not validation_module.validate_token(user_id,validation_token):
            log.info("validation_token is invalid!")
            return False
    return True