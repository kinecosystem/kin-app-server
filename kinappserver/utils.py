...
def print_creation_statement():
    ...
    from .models import BlackhawkCard, BlackhawkOffer, BlackhawkCreds, UserAppData, User, ACL, BackupQuestion, PhoneBackupHints, EmailTemplate, TruexBlacklistedUser, BlacklistedEncPhoneNumber, Task2, SystemConfig, Category, AppDiscovery, AppDiscoveryCategory, Topic
    ...
    log.info(CreateTable(AppDiscovery.__table__).compile(dialect=postgresql.dialect()))
    log.info(CreateTable(AppDiscoveryCategory.__table__).compile(dialect=postgresql.dialect()))
    log.info(CreateTable(Topic.__table__).compile(dialect=postgresql.dialect()))
