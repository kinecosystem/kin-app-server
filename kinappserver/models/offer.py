    ...
    try:
        ...
        for offer in redeemable_offers:
            if os_type == OS_ANDROID:
                if None not in (offer.unavailable_reason, offer.cannot_buy_reason) and LooseVersion(client_version) < LooseVersion(config.GIFTCARDS_ANDROID_VERSION):
                ...
                if None not in (offer.unavailable_reason, offer.cannot_buy_reason) and LooseVersion(client_version) < LooseVersion(config.GIFTCARDS_IOS_VERSION):
                    # client does not support text on offers, remove it
                    offers_to_remove.append(offer)
