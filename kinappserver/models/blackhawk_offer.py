...
class BlackhawkOffer(db.Model):
    ...
    def __repr__(self):
        return '<offer_id: %s, merchant_id: %s, merchant_template_id: %s, batch_size: %s, denomination: %s, minimum_threshold: %s, updated_at: %s>' % (self.offer_id, self.merchant_id, self.merchant_template_id, self.batch_size, self.denomination, self.processed, self.minimum_threshold, self.updated_at)


def create_bh_offer(offer_id, merchant_id, merchant_template_id, batch_size, denomination, minimum_threshold):
...
def list_bh_offers():
    ...
    for offer in offers:
        response[offer.offer_id] = {'merchant_id': offer.merchant_id,
